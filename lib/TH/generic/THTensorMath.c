...
void THTensor_(maskedCopy)(THTensor *tensor, THByteTensor *mask, THTensor* src )
{
  ...
  LONG_PTR cntr = 0;
  LONG_PTR nelem = THTensor_(nElement)(srct);
...
{
  LONG_PTR numel = THByteTensor_sumall(mask);
  real *tensor_data;
...
void THTensor_(gather)(THTensor *tensor, THTensor *src, int dim, THLongTensor *index)
{
  ...
  TH_TENSOR_DIM_APPLY3(real, tensor, real, src, LONG_PTR, index, dim,
...
void THTensor_(scatterFill)(THTensor *tensor, int dim, THLongTensor *index, real val)
{
  ...
  TH_TENSOR_DIM_APPLY2(real, tensor, LONG_PTR, index, dim,
                       for (i = 0; i < elems_per_row; ++i)
                       {
                         idx = *(index_data + i*index_stride);
                         if (idx < 1 || idx > tensor_size) THError("Invalid index in scatter");
...
accreal THTensor_(dot)(THTensor *tensor, THTensor *src)
{
  ...
  TH_TENSOR_APPLY2(real, tensor, real, src,
                   LONG_PTR sz = (tensor_size-tensor_i < src_size-src_i ? tensor_size-tensor_i : src_size-src_i);
...
void THTensor_(add)(THTensor *r_, THTensor *t, real value)
{
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) && THTensor_(nElement)(r_) == THTensor_(nElement)(t)) {
      ...
      LONG_PTR sz = THTensor_(nElement)(t);
      long i;
...
{
  LONG_PTR N1 = m1->size[0];
  LONG_PTR N2 = m2->size[0];
  LONG_PTR dim;
  real *m1_p;
  real *m2_p;
  real *r_p;
...
void THTensor_(addbmm)(THTensor *result, real beta, THTensor *t, real alpha, THTensor *batch1, THTensor *batch2)
{
  ...
  LONG_PTR dim1 = THTensor_(size)(batch1, 1);
  LONG_PTR dim2 = THTensor_(size)(batch2, 2);
...
void THTensor_(baddbmm)(THTensor *result, real beta, THTensor *t, real alpha, THTensor *batch1, THTensor *batch2)
{
  ...
  LONG_PTR bs = THTensor_(size)(batch1, 0);
  LONG_PTR dim1 = THTensor_(size)(batch1, 1);
  LONG_PTR dim2 = THTensor_(size)(batch2, 2);
...
LONG_PTR THTensor_(numel)(THTensor *t)
{
...
void THTensor_(max)(THTensor *values_, THLongTensor *indices_, THTensor *t, int dimension)
{
  ...
  TH_TENSOR_DIM_APPLY3(real, t, real, values_, LONG_PTR, indices_, dimension,
                       LONG_PTR theIndex = 0;
...
accreal THTensor_(trace)(THTensor *t)
{
  ...
  accreal sum = 0;
  LONG_PTR i = 0;
  LONG_PTR t_stride_0, t_stride_1, t_diag_size;
...
void THTensor_(diag)(THTensor *r_, THTensor *t, int k)
{
  ...
  {
    real *t_data = THTensor_(data)(t);
    LONG_PTR t_stride_0 = THTensor_(stride)(t, 0);
    LONG_PTR t_size = THTensor_(size)(t, 0);
    LONG_PTR sz = t_size + (k >= 0 ? k : -k);
    real *r__data;
    LONG_PTR r__stride_0;
    LONG_PTR r__stride_1;
    LONG_PTR i;
  ...
  else
  {
    real *t_data = THTensor_(data)(t);
    LONG_PTR t_stride_0 = THTensor_(stride)(t, 0);
    LONG_PTR t_stride_1 = THTensor_(stride)(t, 1);
    LONG_PTR sz;
    real *r__data;
    LONG_PTR r__stride_0;
    LONG_PTR i;
...
void THTensor_(eye)(THTensor *r_, LONG_PTR n, LONG_PTR m)
{
  real *r__data;
  LONG_PTR i, sz;
...
void THTensor_(range)(THTensor *r_, accreal xmin, accreal xmax, accreal step)
{
  ...
  size = (LONG_PTR)((xmax/step - xmin/step)+1);
...
void THTensor_(randperm)(THTensor *r_, THGenerator *_generator, LONG_PTR n)
{
  real *r__data;
  LONG_PTR r__stride_0;
  LONG_PTR i;
  ...
  for(i = 0; i < n-1; i++)
  {    
    LONG_PTR z = THRandom_random(_generator) % (n-i);
...
static void THTensor_(quicksortascend)(real *arr, LONG_PTR *idx, LONG_PTR elements, LONG_PTR stride)
{
  LONG_PTR beg[MAX_LEVELS], end[MAX_LEVELS], i, j, L, R, P, swap, pid, stack = 0, sz_right, sz_left;
  real rswap, piv;
...
static void THTensor_(quicksortdescend)(real *arr, LONG_PTR *idx, LONG_PTR elements, LONG_PTR stride)
{
  LONG_PTR beg[MAX_LEVELS], end[MAX_LEVELS], i, j, L, R, P, swap, pid, stack = 0, sz_right, sz_left;
  real rswap, piv;
  unsigned char done = 0;
...
void THTensor_(sort)(THTensor *rt_, THLongTensor *ri_, THTensor *t, int dimension, int descendingOrder)
{
  ...
  if(descendingOrder)
  {
    TH_TENSOR_DIM_APPLY2(real, rt_, LONG_PTR, ri_, dimension, 
                         LONG_PTR i;
...
static void THTensor_(quickselect)(real *arr, LONG_PTR *idx, LONG_PTR k, LONG_PTR elements, LONG_PTR stride)
{
  LONG_PTR P, L, R, i, j, swap, pid;
  real rswap, piv;
  L = 0;
  R = elements-1;
...
void THTensor_(kthvalue)(THTensor *values_, THLongTensor *indices_, THTensor *t, LONG_PTR k, int dimension)
{
  THLongStorage *dim;
  THTensor *temp_;
  THLongTensor *tempi_;
  real *temp__data;
  LONG_PTR *tempi__data;
  LONG_PTR t_size_dim;
...
void THTensor_(tril)(THTensor *r_, THTensor *t, LONG_PTR k)
{
  LONG_PTR t_size_0, t_size_1;
  LONG_PTR t_stride_0, t_stride_1;
  LONG_PTR r__stride_0, r__stride_1;
  real *t_data, *r__data;
  LONG_PTR r, c;
  ...
  {
    LONG_PTR sz = THMin(r+k+1, t_size_1);
...
void THTensor_(triu)(THTensor *r_, THTensor *t, LONG_PTR k)
{
  LONG_PTR t_size_0, t_size_1;
  LONG_PTR t_stride_0, t_stride_1;
  LONG_PTR r__stride_0, r__stride_1;
  ...
  for(r = 0; r < t_size_0; r++)
  {
    LONG_PTR sz = THMin(r+k, t_size_1);
    for(c = THMax(0, r+k); c < t_size_1; c++)
      r__data[r*r__stride_0+c*r__stride_1] = t_data[r*t_stride_0+c*t_stride_1];
    for(c = 0; c < sz; c++)
      r__data[r*r__stride_0+c*r__stride_1] = 0;
...
void THTensor_(cat)(THTensor *r_, THTensor *ta, THTensor *tb, int dimension)
{
  ...
  for(i = 0; i < ndim; i++)
  {
    LONG_PTR tadi = (i < ta->nDimension ? ta->size[i] : 1);
    LONG_PTR tbdi = (i < tb->nDimension ? tb->size[i] : 1);
...
void THTensor_(linspace)(THTensor *r_, real a, real b, LONG_PTR n)
{
  real i = 0;
...
void THTensor_(logspace)(THTensor *r_, real a, real b, LONG_PTR n)
{
  real i = 0;
...
void THTensor_(histc)(THTensor *hist, THTensor *tensor, LONG_PTR nbins, real minvalue, real maxvalue)
{
  THTensor *clone;
  real minval;
  real maxval;
